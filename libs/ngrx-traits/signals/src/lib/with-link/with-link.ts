 ...
 * linked signal uses (the `update` callback, or `patchState` by default),
...
> = {
   ...
   * produces fresh object/array references (e.g. entity selections): either a
   * function, or the name of a premade one — 'array' (shallow, order
   * sensitive), 'set' (order insensitive, both only offered for arrays),
   * 'stringify' (JSON.stringify, works on objects and arrays), a property of
   * the value to compare by, e.g. 'id', or 'array.id' / 'set.id' to compare
   * the elements of an array by one of their properties.
   */
 ...
 * `update` transforms the value. Besides a function, `equal` accepts the name
 * of a premade comparison: `'array'` (shallow, order sensitive), `'set'`
 * (order insensitive — both only offered when the value is an array),
 * `'stringify'` (JSON.stringify, for objects and arrays), a property of the
 * value to compare by, or `'array.<prop>'` / `'set.<prop>'` to compare the
 * elements of an array by one of their properties.
 *
  ...
  return withFeatureFactory((store) => {
    ...
    return signalStoreFeature(
      withMethods(() => ({
        ...(options?.noSetter
          ? {}
          : { [setterMethodName]: linkSetter(source, guardedUpdate) }),
 ...
 * link's update path, that also accepts an updater receiving the current value.
 */
function linkSetter<T>(
  source: () => T,
  update: (value: T) => void,
): LinkSetter<T> {
  const setValue = signalMethod<T>((value) => update(value));
  ...
  const setter = (
    ...
    if (typeof value === 'function' && value.length > 0) {
      update((value as (current: T) => T)(untracked(source)));
