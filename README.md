...
<!-- I was thinking in explaining what is a trait is  -->
<!-- Do you think that forth mentioning the factory functions? or some synonim that people that reads this won't have to go and make sure what is a factory  -->

NGRX Traits is a set of factory functions that dynamically generate actions, effects, reducers needed for common functionality like loading a list of entities, filtering, pagination, sorting, etc. They can be mixed and matched, so you only use what you need, and you can create your own too, for example, reuse some business logic across your app.
<!-- What about something like you can compose them in any way you choose  -->
...
##Getting Started
The best way to understand how to use the traits is to see an example. Lets imagine that you need to implement a page that shows a list of products which you can select and buy Start by creating the interface for our Product entity:
...
To check what was created by the factory, using your prefeerred IDE intellisense, inspect the `productTraits` variable
...
The most important ones you will see there are the fetch actions. They are meant to be used to create an effect that will populate the state. These actions are also used by other entities related traits like filter, sort, pagination, to fetch data when needed. Let's do one for our product list:
...
In order to be ready to be able to use the traits create a module for the state.
