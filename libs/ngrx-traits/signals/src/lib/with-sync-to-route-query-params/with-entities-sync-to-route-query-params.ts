 ...
 *
 * @param config - The full feature config or — in the two-argument form — just the entityConfig (`entityConfig({ entity, collection })`)
 ...
 *
 * @example
 * // this feature only covers the entities params, any other state that should
 * // survive a reload is synced by adding withSyncToRouteQueryParams with a
 * // mapper generated by getQueryMapperForState, the two live side by side
 * const productEntityConfig = entityConfig({
 *   entity: type<Product>(),
 *   collection: 'product',
 * });
 ...
 *     day: new Date(),
 *   }),
 *   // 👇 product-filter, product-page, product-pageSize, product-selectedId
 *   withEntitiesSyncToRouteQueryParams(productEntityConfig),
 *   // 👇 view, showSold and day, the prop names and their types are checked
 ...
 *   }),
 *);
 * // ?product-filter=%7B%22search%22%3A%22tv%22%7D&product-page=2
 * //  &view=grid&showSold=true&day=2026-08-11
 *
 * @example
 * // the filter is serialized with JSON.stringify by default, which makes it
 * // the longest param in the url. getFilterQueryMapper builds a filterMapper
 * // that spreads it over one param per field instead, the filter type has to
 * // be given since it cannot be inferred from the store
 ...
 *   withCallStatus(productEntityConfig, { initialValue: 'loading' }),
 *   withEntitiesRemoteFilter(productEntityConfig, {
 *     defaultFilter: { search: '', maxPrice: 0, from: new Date() },
 *   }),
 *   withEntitiesLoadingCall(productEntityConfig, { fetchEntities: () => ... }),
 *   withEntitiesSyncToRouteQueryParams(productEntityConfig, {
 ...
 *   }),
 *);
 * // ?product-search=tv&product-maxPrice=100&product-from=2026-08-11
 * // instead of ?product-filter=%7B%22search%22%3A%22tv%22%2C%22maxPrice%22...
 */
...
>(
  ...
  const config = { ...configOrEntityConfig, ...options } as {
    entity: Entity;
    collection?: Collection;
    filterMapper?: FilterQueryMapper<Filter>;
    prefix?: string | false;
