...
### Using it with the Angular Resource view

There is no separate resource for a page. `[collection]EntitiesCurrentPage` already carries the entities, `total`, `pageIndex`, `pageSize` and `isLoading`, and being a `DeepSignal` each prop can be read on its own (`productEntitiesCurrentPage.total()`). Pair it with the resource generated by [withEntitiesLoadingCall](/docs/traits/with-entities-loading-call#with-a-pagination-store-feature) when you want `status()` and `error()`: the rows and the paginator come from the page, since the entity collection only holds the cached page window.
...
```typescript
usersCurrentPage: DeepSignal<{ entities: Product[]; pageIndex: number; total: number; pageSize: 5; pagesCount: number; hasPrevious: boolean; hasNext: boolean; isLoading: boolean }>;
